...
packages = c('rmarkdown','gganimate','shiny','tidyverse','shinydashboard','lubridate','plotly','leaflet','readr','crosstalk','shinythemes','sparkline','skimr','shinycssloaders','ggcorrplot','RColorBrewer','rgdal','vip','SnowballC','slam','zoo',"bslib", "ggplot2",
             ...
             'ggthemes', 'ggiraph', "shinyWidgets","networkD3")
...
write.csv(singapore,"singapore.csv", row.names = FALSE)
```
```{r}
...
fig <- fig %>%
  layout(title = 'Trade Volume, Singapore (Jan 2018-Jan 2022)\nSharp Dip in Trade Volume in May 2020', xaxis = list(
      range= c(0,50), showticklabels = FALSE),yaxis = list( title = "Trade Volume", range= c(50000000000,100000000000),showgrid = FALSE),autosize = T, width = 800, height = 500)
...
fig <- fig %>%
  layout(title = 'Trade Volume, Singapore (Jan 2018 - Jan 2022)', xaxis = list(), yaxis = list(),autosize = F, width = 900, height = 500)
fig <- fig %>%
  animation_opts(
...
p <- p+geom_line(singapore, mapping = aes(x = factor(Year_Month,level = unique(Year_Month)), y=BalanceOfTrade, group = 1))+ annotate("text", x=25, y=20, label = "Balance of Trade", color = "white")
...
t <- ggplot(singapore, aes(x = Year_Month,y=Trade_Volume, frame = Year_Month,  size = 10, color = "red",text = paste0("Time: ", Year_Month))) +  geom_point()


t <- t +labs(title = "Singapore Trade Volume Trend ( Jan 2018 to Jan 2022)", 
                     
                          subtitle = "Sharp Dip in May 2020",
                     x = 'Year Month', 
                     y ='Trade Volume')
  

t <- t + theme_dendro() + theme(legend.position = "none",plot.title = element_text(size=10), 
                            plot.subtitle = element_text(size=9), axis.title = element_text(size=8, hjust=0.5), 
                            axis.text.x=element_blank(),  axis.text.y=element_blank())

t <- ggplotly(t, tooltip = ("text"))
...
jan2018 <- trade[c("Imports", "Country", "Year_Month","Origin")]%>%
  filter(Year_Month == "2018 Jan")
jan2018 <- jan2018[order(jan2018$Imports, decreasing = TRUE), ]
jan2018 <- head(jan2018,10)
view(jan2018)

```
```{r}
name_vec <- c(unique(jan2018$Origin), unique(jan2018$Country))
...
jan20181 <- jan2018 %>%
  ...
  rename(origin_id = id)
view(jan20181)
sankeyNetwork(Links = jan20181[c("origin_id","country_id","Imports")],Nodes = name_df, Source = "origin_id", Target = "country_id", Value = "Imports", NodeID = "name",fontSize = 12, fontFamily = 'Arial', nodeWidth = 20)
```
```{r}
#view(trade)
...
output$network <- renderPlot({
    if (input$tradetype1 == "Imports"){
      ...
      trade2 <-trade[c(input$tradetype1, "Country", "Year_Month","Origin")] %>%
        filter(Year_Month %in% input$Time2)
      trade2 <- trade2[order(input$tradetype1, decreasing = TRUE), ]
      trade2 <- head(trade2,10)
      
      name_vec <- c(unique(trade2$Origin), unique(trade2$Country))
      name_df <- data.frame("name"=name_vec, "id"=0:10)
      ...
      sankeyNetwork(Links = trade3[c("origin_id","country_id",input$tradetype1)],
